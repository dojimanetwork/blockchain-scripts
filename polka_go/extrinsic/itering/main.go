...
func ConnectDot() {
	iws.SetEndpoint(local)
}

func main() {
	ConnectDot()
	iapi, err := iws.Init()
	iapi.Conn.Dial(local, http.Header{})
	...
	err = iws.SendWsRequest(nil, blockResult, irpc.ChainGetBlockHash(blockNumber, blockNumber))
	if err != nil {
	...
	err = iws.SendWsRequest(nil, v, irpc.ChainGetBlock(44410, blockHash))
	...
	decodedExtrinsics, _ := decodeExtrinsics(rpcBlock.Block.Extrinsics, currentMetadata, 14)
...
func decodeExtrinsics(list []string, metadata *iMetadata.Instant, spec int) (r []iScale.ExtrinsicDecoder, err error) {
	...
	for _, extrinsicRaw := range list {
		...
		fmt.Println(info.PartialFee, err)
